...
本目录通过 C++23 Notebook 学习拥有型连续容器、非拥有视图、可选值以及它们在 Native SDK 边界上的生命周期规则。
...
| 6 | [`std::optional`](06_optional.ipynb) | 显式缺失状态、值生命周期和 C 状态码映射 |
...
完成本部分后，应能根据长度是否固定、数据是否需要拥有、借用持续多久，选择 `string`、`string_view`、`vector`、`array` 或 `span`；并能用 `optional` 表达正常缺失，同时解释为什么 C ABI 不能直接暴露这些 C++ 类型。
